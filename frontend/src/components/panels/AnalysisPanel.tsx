...
import { Brain, TrendingUp, TrendingDown, Target, DollarSign, Clock, BarChart3, Sparkles } from 'lucide-react';
...
export default function AnalysisPanel() {
  ...
  const fetchSummary = useCallback(async () => {
    setLoading(true);
    try {
      const response = await axios.get('/api/analysis/trade-summary?use_ai=false');
  ...
  useEffect(() => {
    fetchSummary();
  }, [_analysisRefreshKey, fetchSummary]); // 当刷新键变化时重新获取

  // 获取AI分析（调用AI，传入开仓历史和资金曲线）
  const fetchAiAnalysis = async () => {
    setAiLoading(true);
    try {
      const response = await axios.get('/api/analysis/trade-summary?use_ai=true');
      setAnalysis(response.data);
    } catch (error) {
  ...
  if (!analysis) {
    return (
      <div className="jojo-card p-6 text-center text-gray-400">
        <p className="mb-4">无法获取统计摘要</p>
        <button
  ...
  return (
    <div className="jojo-card jojo-card-no-scale p-4">
      {/* 标题区域 */}
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-jojo-gold/30">
